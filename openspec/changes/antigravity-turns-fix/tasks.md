...
- [ ] 2.1 在 `internal/transcript/antigravity_transcript_test.go` 中新增測試，驗證從 `settings.json` 讀取並常態化 model 名稱，並驗證當 transcript 內無 token 數據時，不報錯且回傳 token 消耗為 0
- [ ] 2.2 在 `internal/transcript/antigravity_transcript.go` 中實作 `getAntigravityModel()` 函數，讀取並解析 `~/.gemini/antigravity-cli/settings.json`（若不存在則 fallback 至 `~/.gemini/antigravity/settings.json`）
- [ ] 2.3 修改 `ParseAntigravityLog`，使用常態化的 model 名稱，並回傳 input/output token 消耗均為 0 的 `WindowResult`
- [ ] 2.4 執行測試 `go test ./internal/transcript/...` 並確認測試全部通過
