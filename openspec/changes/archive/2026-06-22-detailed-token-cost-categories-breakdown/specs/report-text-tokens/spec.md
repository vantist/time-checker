...
系統 SHALL 透過 `tt report` 命令的 `--output` / `-o` 參數支援將報表內容直接寫入指定檔案，不輸出至標準輸出。

#### Scenario: 指定 --output 寫入純文字檔案

- **WHEN** 使用者執行 `tt report --output report.txt`
- **THEN** `report.txt` 檔案被建立，內容為 FormatText 格式的純文字報表
- **THEN** 檔案權限為 `0600`
...
- **WHEN** DB 中無符合條件 of session
- **THEN** 四個 token 欄位 SHALL 顯示 `0`，Est. cost 顯示 `$0.0000`

### Requirement: 文字報告依 Project 分組

`tt report` 的文字輸出 SHALL 在 tokens 區塊後加入 By Project 區塊，以表格形式每行顯示 project 名稱（使用 `path.Base`）、session 數、agent time、user active time、tokens 數量 (格式為 `input / output / cache read / cache create`)、以及 est. cost。

#### Scenario: 多 project 分組顯示

- **WHEN** sessions 跨多個 project
- **THEN** By Project 區塊以表格對齊顯示，欄位標頭包含 `Project`、`Sessions`、`Agent Time`、`User Active`、`Tokens`、`Cost`
...
- **THEN** JSON 根物件 MUST 含 `by_project` 陣列，每個元素含 `project`（string）、`sessions`（int）、`agent_time_seconds`（int）、`cost_usd`（float，可為 null）、`input_tokens`（int）、`output_tokens`（int）、`cache_read_tokens`（int）、`cache_creation_tokens`（int）
...
`tt report` 的文字輸出 SHALL 在 tokens/cost 區塊後加入 Daily 統計表格，顯示過去 7 天的每日 session 數與 token 使用量（含 Input, Output, Cache read, Cache create）。

#### Scenario: Daily 統計表渲染

- **WHEN** 過去 7 天有 session 資料且執行 `tt report`
- **THEN** 輸出包含 `─── Daily (Last 7 Days) ───` 標題行，與包含 `Date`、`Sessions`、`Input`、`Output`、`Cache Rd`、`Cache Cr` 標頭的表格
- **THEN** 每行依日期升序排列，欄位對齊顯示，數字以千分位逗號格式化

### Requirement: 文字報告顯示 By Work Item 分組表

`tt report` 的文字輸出 SHALL 在 `By Project` 之後加入 By Work Item 分組表格（若有多於 1 個 work item 分組或使用者在 CLI 中指定 `--by-work-item` 參數時），表格中應包含 `Tokens` 欄位。

#### Scenario: By Work Item 表格渲染

- **WHEN** 存在多個不同的 work item 分組（包含 `untagged`）或使用者指定了 `--by-work-item`
- **THEN** 輸出包含 `─── By Work Item ───` 標題行，與包含 `Work Item`、`Project`、`Sessions`、`Agent Time`、`User Active`、`Tokens`、`Cost` 標頭的表格
- **THEN** Tokens 欄位格式為 `input / output / cache read / cache create`
- **THEN** 每列資料對齊顯示，依 Agent Time 降序排列

### Requirement: 文字報告顯示詳細 Sessions 日誌表

`tt report` 的文字輸出 SHALL 在最後加入詳細的 Sessions 日誌表格，列出查詢範圍內的所有 sessions，且包含 `Tokens` 欄位。

#### Scenario: Sessions 日誌表渲染

- **WHEN** 查詢範圍內有 session 資料且執行 `tt report`
- **THEN** 輸出包含 `─── Sessions ───` 標題行，與包含 `Start Time`、`Project`、`Branch`、`Agent`、`Model`、`Turns`、`Agent Time`、`User Time`、`Work Item`、`Tokens`、`Cost` 標頭的表格
- **THEN** Tokens 欄位格式為 `input / output / cache read / cache create`
